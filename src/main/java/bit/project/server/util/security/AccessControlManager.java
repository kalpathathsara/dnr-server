...
public class AccessControlManager {
    ...
    private Token getActiveToken(HttpServletRequest request){
        ...
        try{
            ...
            // Parse the token using the new API
//            Claims claims = Jwts.parserBuilder()
//                    .setSigningKey(key) // Set the signing key
//                    .build()            // Build the JwtParser
//                    .parseClaimsJws(tokenString) // Parse the token
//                    .getBody();
            Claims body = Jwts.parser().setSigningKey(SECRET_KEY).build().parseClaimsJws(tokenString).getBody();
    ...
    private String generateToken(Token token) {
        Claims claims = (Claims) Jwts.claims().setSubject(token.getId());
